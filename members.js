...
async function renderMemberDetail(container, name) {
    ...
    // Update Section Title and Description for Detail View
    const sectionTitle = document.querySelector('.section-title');
    const sectionDesc = document.querySelector('.section .text-center');
    if (sectionTitle) sectionTitle.innerText = '自己紹介カード';
    if (sectionDesc) sectionDesc.innerText = 'この人が書いた記事は下に一覧があるよ！';
...
function renderSingleMemberCard(member, isDetail) {
    ...
    if (isDetail) {
        ...
        return `
        <a href="members.html?name=${encodeURIComponent(member.name)}" class="member-card">
            <div class="member-card-line"></div>
            ...
            <div class="member-card-body">
                <div class="member-header-group">
                    <h3 class="member-list-name">★ ${member.name}</h3>
                    ${(member.Instagram || member.instagram_url || member.Instagram_url) ? `
                    <div class="member-instagram-btn" onclick="event.preventDefault(); event.stopPropagation(); window.open('${member.Instagram || member.instagram_url || member.Instagram_url}', '_blank');" title="Instagramを見る">
                ...
                ${member.birth_year ? `<p class="member-birth-year">${member.birth_year}</p>` : ''}
